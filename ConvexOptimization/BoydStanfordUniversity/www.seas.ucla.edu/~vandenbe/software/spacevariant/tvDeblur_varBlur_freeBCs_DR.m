function [xPlus_x,costs,errors] = tvDeblur_varBlur_freeBCs_DR(b,kernels,U,params,xStar)
...
% We solve minimize .5*||Kx - bHat ||^2 + gamma*||Dx||_1
% K = U1*K1 + ... + U_P*K_P.
% Each Kp convolves using periodic BCs.
% D is analysis operator for a shearlet tight frame.
% Ui zeros out pixels on the border, and multiplied remaining components
...
%%% Now we're minimizing .5|| Kx - b||^2 + gamma ||Dx||_1.
%%% Note that original objective function has been multiplied by beta^2.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     proxg2Star = max(proxg2Star,-gamma);
    ...
    Dx = applyD(xPlus_x);
    cost = .5*sum(sum((Kx - b).^2)) + gamma*evalIsoNorm(Dx);
    cost = cost/beta^2;
    costs = [costs,cost];
